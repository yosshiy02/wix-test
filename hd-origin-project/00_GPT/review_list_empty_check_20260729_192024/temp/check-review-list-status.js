...
async function main() {
  ...
  if (!ocrColumns.length) {
    throw new Error(
      "payment_document_ocr_importsが見つかりません。"
    );
  }

  if (!basicColumns.length) {
    throw new Error(
      "payment_document_basic_analysis_resultsが見つかりません。"
    );
  }
  ...
  if (
    !ocrColumns.includes("current_status")
  ) {
    throw new Error(
      "OCR取込テーブルにcurrent_statusがありません。"
    );
  }
  ...
  if (
    ...
    print(
      "DIAGNOSIS",
      "DBには基礎解析済みが存在します。一覧APIの会社条件または取得条件に問題があります。"
    );
  } else {
    print(
      "CAUSE_CODE",
