...
/* PAYMENT_DOCUMENT_AI_PROMPT_PRECISION_20260707_START */
function paymentDocumentMasterCodeInstructionText() {
  return [
    "",
    "【最重要: select項目は表示名ではなくマスタコードで返す】",
    "画面のselect項目に入れる値は、日本語の表示名ではなく、下記のコードを返してください。",
    "例: リース契約書 → lease_contract",
    "例: 税金・公的支払い → tax_public",
    "例: 契約・保険・リース → contract_insurance_lease",
    "",
    "document_type_code / 書類区分 / 書類種別 のコード候補:",
    "- invoice: 請求書",
    "- receipt: 領収書",
    ...
    "- other: その他",
    "",
    "payment_destination_code / 処理先 のコード候補:",
    "- payable: 支払処理",
    ...
    "- needs_review: 要確認",
    "",
    "accounting_category_code / 会計区分 のコード候補:",
    "- normal: 通常",
    "- tax: 税金",
    "- public_utility: 公共料金",
    "- insurance: 保険",
    ...
    "",
    "分類ルール:",
    "- OCR本文に「リース契約書」「月額リース料」「貸主」「借主」「契約期間」があれば lease_contract / contract_insurance_lease / lease を優先。",
    "- OCR本文に「納付書」「納税通知書」「納付先」「税目」「税務署」「法人税」「合計納付額」があれば tax_payment_notice / tax_public / tax を優先。",
    "- OCR本文に「カード」「ご利用明細」「引落日」があれば card_statement / card_payable を優先。",
    "- OCR本文に「電気」「ガス」「水道」「使用量」「お客様番号」があれば utility_notice / public_utility を優先。",
    "- OCR本文に「保険」「保険料」「保険期間」があれば insurance_notice / contract_insurance_lease / insurance を優先。",
    "",
    "fields内のselect系ラベルへ入れる値:",
    "- 書類種別: 上記 document_type_code のコード",
    "- 書類区分: 上記 document_type_code のコード",
    "- 処理先: 上記 payment_destination_code のコード",
    "- 会計区分: 上記 accounting_category_code のコード",
    "- 未払種別: 上記 payable_kind_code のコード",
    ...
    "",
    "自由入力でよい項目:",
    "- 書類名、発行元、支払先、宛名、会社名、住所、番号、日付、金額、摘要、社内メモ、要確認メモ",
    "",
    "禁止:",
...
function injectPaymentDocumentPromptPrecision(prompt) {
  const instruction = paymentDocumentMasterCodeInstructionText();

  if (String(prompt || "").includes("【最重要: select項目は表示名ではなくマスタコードで返す】")) {
    return prompt;
  }

  return String(prompt || "") + "\n" + instruction + "\n";
}
/* PAYMENT_DOCUMENT_AI_PROMPT_PRECISION_20260707_END */
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "OCR本文だけから、支払書類の会計入力候補JSONを作成してください。必ずJSONのみを返してください。"
        },
        {
          role: "user",
          content: prompt
...
function normalizeAiDraftCandidate(value) {
  ...
  return {
    document_type_code: normalizeAiValueText(draft.document_type_code),
    payment_destination_code: normalizeAiValueText(draft.payment_destination_code),
    ...
    ai_summary: {
      document_kind: normalizeAiValueText(draft.ai_summary && draft.ai_summary.document_kind || ""),
...
function buildPaymentDocumentAiPrompt(ocrText) {
  const fieldNames = [
    "書類種別", "処理先", "支払対象", "未払登録対象", "経費登録対象", "税金・公的支払", "契約・保険・リース", "AI信頼度", "AI判定理由",
    "書類区分", "証憑区分", "書類名", "発行元", "支払先", "宛名", "会社名", "個人名", "部署名", "担当者名", "住所", "電話番号", "メール", "Webサイト",
...
function buildPaymentDocumentClassificationPrompt(ocrText) {
  return injectPaymentDocumentPromptPrecision([
    "あなたは日本の中小企業向け会計入力補助AIです。",
...
async function createTwoStepAiDraftFromOcrText(ocrText) {
  ...
  const classification = applyPaymentDocumentRuleFallbackFromOcr(ocrText, normalizeAiDraftCandidate(classificationResponse.parsed));
  ...
  const draft = applyPaymentDocumentRuleFallbackFromOcr(ocrText, normalizeAiDraftCandidate(mergedRaw));
  draft.visible_field_labels = visibleLabels;
  draft.document_group = group;

  return {
    draft,
    classification,
    document_group: group,
    visible_field_labels: visibleLabels,
