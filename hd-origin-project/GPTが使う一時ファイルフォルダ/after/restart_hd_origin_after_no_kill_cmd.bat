...
rem OLD_CMD_PID ‚Í Chrome Šª‚«ž‚Ý–hŽ~‚Ì‚½‚ßI—¹‚µ‚È‚¢
rem if defined OLD_CMD_PID (
rem   echo Closing old launcher window...
rem   taskkill /PID %OLD_CMD_PID% /T /F >nul 2>nul
rem )
...
if not exist "%START_BAT%" (
  echo start_hd_origin.bat was not found.
  echo %START_BAT%
  pause
  exit /b 1
)
