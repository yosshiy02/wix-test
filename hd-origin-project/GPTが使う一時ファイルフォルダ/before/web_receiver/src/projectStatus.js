...
function writeProjectStatus() {
  ...
  lines.push("- GPTに見せるファイルは、プロジェクト直下の「GPTが見たいファイル一時フォルダ」に集める。");
  lines.push("- ファイル収集時は、この一時フォルダ内を先にクリーンしてから必要ファイルだけコピーする。");
  lines.push("- もともと入っていたファイルは、前回作業や別作業の関係ないファイルとして扱う。");
  lines.push("- 収集対象の元ファイルは変更しない。コピーのみ行う。");
  lines.push("");
  lines.push("[生成情報]");
