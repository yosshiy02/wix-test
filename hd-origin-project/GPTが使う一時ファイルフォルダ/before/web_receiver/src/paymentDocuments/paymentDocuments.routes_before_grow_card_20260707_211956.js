...
function applyPaymentDocumentSortRuleFallbackFromOcr(ocrText, draft) {
  ...
  if (tax) {
    ...
    setIfBlank("specialist_route_label", "カード明細解析");
  } else if (receipt) {
  ...
  if (req.method === "POST") {
    ...
    if (sortUrlPath.startsWith("/api/payment-documents/ai-sort/")) {
      try {
        ...
        const sortResult = hdOriginPolishPaymentDocumentSortResult(await createPaymentDocumentSortFromOcrText(ocrText), ocrText);
