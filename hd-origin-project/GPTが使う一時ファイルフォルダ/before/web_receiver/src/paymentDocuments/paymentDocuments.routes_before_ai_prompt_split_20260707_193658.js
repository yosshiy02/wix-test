...
const db = require("../db");
const { loadPaymentDocumentPromptText, appendPaymentDocumentExternalPrompt } = require("./paymentDocuments.aiPromptLoader");
...
async function createTwoStepAiDraftFromOcrText(ocrText) {
  const classificationResponse = await callPaymentDocumentOpenAiJson(
    appendPaymentDocumentExternalPrompt(
      ...
      ["business-rules.txt", "classification.extra-rules.txt"]
    ),
  ...
  const detailResponse = await callPaymentDocumentOpenAiJson(
    appendPaymentDocumentExternalPrompt(
      ...
      ["business-rules.txt", "detail.extra-rules.txt"]
    ),
    loadPaymentDocumentPromptText(
      "detail.system.txt",
