...
async function analyzeReceiptImport(receiptImport) {
  ...
  const instruction = [
    ...
    "receiptTimeText には、レシートに印字されている時刻がある場合だけ HH:mm 形式で入れてください。読めない場合、または印字がない場合は null にしてください。推測で補完しないでください。",
  ...
  return {
    ...
    receiptTimeText: normalizeReceiptTimeText(parsed.receiptTimeText || parsed.receipt_time_text),
