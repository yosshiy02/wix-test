...
async function analyzeReceiptImport(receiptImport) {
  ...
  const instruction = [
    ...
    "receiptTimeText が読める場合は、レシート時刻として返してください。memoにも必要に応じて「レシート時刻 HH:mm」と入れてください。",
    "例: ロース定食2点、2名、18:03 の場合は、summary は「飲食代」、accountTitleName は「会議費」、memo は「飲食代。2名。レシート時刻18:03。出張・打合せ目的・相手先・割り勘・自社負担額の確認が必要。」のようにしてください。",
...
analyzeReceiptImport = async function analyzeReceiptImportWithMasterHints(receiptImport) {
  ...
  const instruction = [
    ...
    "vendorPhone には、レシートに印字されている電話番号を入れてください。読めない場合は空文字にしてください。",
    "receiptTimeText には、レシートに印字されている時刻を HH:mm 形式で入れてください。読めない場合は空文字にしてください。",
    "receiptTimeText に入れた時刻は、memo に重複して書かないでください。",
    "伝票番号、注文番号、レジ番号、会員番号、承認番号、カード番号は invoiceNumber に入れず、memo に入れてください。",
    "",
    "【目的判定ルール】",
    ...
    "memo にはレシート時刻を重複して書かないでください。時刻は receiptTimeText に入れてください。",
    "",
    "【インボイス区分ルール】",
