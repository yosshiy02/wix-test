...
/* PAYMENT_DOCUMENT_REVIEW_PREVIEW_ROTATE_20260707_START */
  ...
  function applyDocumentPreviewRotation() {
    ...
    target.classList.remove("preview-rotated-sideways");
    ...
    const containerWidth = Math.max(1, previewRect.width - 24);
    const containerHeight = Math.max(1, previewRect.height - 24);

    const normalizedRotation = ((documentPreviewRotation % 360) + 360) % 360;
    const isSideways = normalizedRotation === 90 || normalizedRotation === 270;

    const baseWidth = Math.max(1, targetRect.width);
    const baseHeight = Math.max(1, targetRect.height);

    const rotatedWidth = isSideways ? baseHeight : baseWidth;
    const rotatedHeight = isSideways ? baseWidth : baseHeight;
    ...
    if (isSideways) {
      target.classList.add("preview-rotated-sideways");
    }

    target.style.transform =
      "rotate(" + normalizedRotation + "deg) scale(" + scale + ")";
    target.style.transformOrigin = "center center";
  }
...
/* PAYMENT_DOCUMENT_REVIEW_PREVIEW_FIT_20260707_START */
...
/* PAYMENT_DOCUMENT_REVIEW_PREVIEW_FIT_20260707_END */
  function setDocumentPreview(item) {
    ...
    if (isPreviewImage(item, url)) {
      preview.innerHTML =
        '<img class="document-preview-image" src="' + esc(url) + '" alt="' + esc(name) + '">';
      return;
    }
