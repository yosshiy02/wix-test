...
/* PAYMENT_DOCUMENT_AI_FIELD_LINK_DEBUG_20260707_START */
  ...
  function setDebugControlValue(control, label, value) {
    ...
    if (control.tagName === "SELECT") {
      ...
      if (!matched) {
        const debugValue = "__AI_DEBUG__" + label;
        let option = Array.from(control.options || []).find(item => item.value === debugValue);

        if (!option) {
          option = document.createElement("option");
          option.value = debugValue;
          option.textContent = text;
          option.dataset.aiDebugOption = "1";
          control.appendChild(option);
        }

        control.value = debugValue;
      }
    } else {
      control.value = text;
    }
  ...
  function applyAiDraftToForm(draft) {
    ...
    // AI判定サマリー
    put("draftAiDocumentKind", "書類種別", summary.document_kind || fields["書類種別"]);
    put("draftAiDestination", "処理先", summary.destination || fields["処理先"]);
    put("draftAiPaymentTarget", "支払対象", summary.payment_target || fields["支払対象"]);
    put("draftAiPayableTarget", "未払登録対象", summary.payable_target || fields["未払登録対象"]);
    put("draftAiExpenseTarget", "経費登録対象", summary.expense_target || fields["経費登録対象"]);
    put("draftAiTaxPublic", "税金・公的支払", summary.tax_public || fields["税金・公的支払"]);
    put("draftAiContractInsuranceLease", "契約・保険・リース", summary.contract_insurance_lease || fields["契約・保険・リース"]);
    put("draftAiConfidence", "AI信頼度", summary.confidence_label || fields["AI信頼度"]);
    put("draftAiReason", "AI判定理由", summary.reason || fields["AI判定理由"]);

    // 基本情報
    put("draftDocumentType", "書類区分", draft.document_type_code || fields["書類区分"]);
    put("draftEvidenceType", "証憑区分", fields["証憑区分"]);
    put("draftDocumentName", "書類名", fields["書類名"] || draft.summary);
    ...
    put("draftPersonalName", "個人名", fields["個人名"]);
    put("draftDepartmentName", "部署名", fields["部署名"]);
    put("draftContactPerson", "担当者名", fields["担当者名"]);
    ...
    // 番号・識別情報
    put("draftInvoiceNumber", "請求書番号", draft.invoice_number || fields["請求書番号"]);
    put("draftReceiptNumber", "領収書番号", fields["領収書番号"]);
    put("draftPaymentNumber", "納付番号", fields["納付番号"]);
    put("draftNoticeNumber", "通知書番号", fields["通知書番号"]);
    put("draftManagementNumber", "管理番号", fields["管理番号"]);
    put("draftCustomerNumber", "お客様番号", fields["お客様番号"]);
    put("draftContractNumber", "契約番号", fields["契約番号"]);
    put("draftMemberNumber", "会員番号", fields["会員番号"]);
    put("draftOrderNumber", "注文番号", fields["注文番号"]);
    put("draftTransactionNumber", "取引番号", fields["取引番号"]);
    put("draftInvoiceRegistrationNumber", "登録番号", fields["登録番号"]);
    put("draftCorporateNumber", "法人番号", fields["法人番号"]);
    ...
    put("draftTaxIncludedAmount", "税込金額", fields["税込金額"] || draft.total_amount);
    put("draftTaxExcludedAmount", "税抜金額", fields["税抜金額"]);
    put("draftTaxAmount", "消費税額", fields["消費税額"] || draft.tax_amount);
    put("draftTaxable10Amount", "10%対象金額", fields["10%対象金額"]);
    put("draftTax10Amount", "10%消費税", fields["10%消費税"]);
    put("draftTaxable8Amount", "8%対象金額", fields["8%対象金額"]);
    put("draftTax8Amount", "8%消費税", fields["8%消費税"]);
    put("draftNonTaxableAmount", "非課税・不課税", fields["非課税・不課税"]);
    ...
    put("draftCurrentUsageAmount", "今回利用額", fields["今回利用額"]);
    put("draftPaidAmount", "入金額", fields["入金額"]);
    put("draftUnpaidBalance", "未払残高", fields["未払残高"]);
    ...
    put("draftWithdrawalBank", "引落銀行", fields["引落銀行"]);
    put("draftCardCompany", "カード会社", fields["カード会社"]);
    put("draftCardName", "カード名", fields["カード名"]);
    put("draftSettlementService", "決済サービス", fields["決済サービス"]);
    put("draftConveniencePaymentNumber", "コンビニ支払番号", fields["コンビニ支払番号"]);
    put("draftBarcodeNumber", "バーコード番号", fields["バーコード番号"]);
    put("draftQrPaymentInfo", "QR決済情報", fields["QR決済情報"]);

    // 会計・管理
    put("draftAccountingCategory", "会計区分", draft.accounting_category_code || fields["会計区分"]);
    put("draftPaymentDestination", "処理先", draft.payment_destination_code || fields["処理先"]);
    ...
    put("draftInvoiceCategory", "インボイス区分", fields["インボイス区分"]);
    ...
    put("draftTaxPaymentDestination", "納付先", fields["納付先"]);
    put("draftFiscalYear", "年度", fields["年度"]);
    put("draftPeriodName", "期別", fields["期別"]);
    put("draftUtilityCustomerNumber", "公共料金お客様番号", fields["公共料金お客様番号"]);
    put("draftUsagePeriod", "使用期間", fields["使用期間"]);
    put("draftUsageAmount", "使用量", fields["使用量"]);
    put("draftInsuranceType", "保険種類", fields["保険種類"]);
    put("draftLeaseItem", "リース物件", fields["リース物件"]);
    put("draftPaymentCount", "支払回数", fields["支払回数"]);
    put("draftMailSubject", "メール件名", fields["メール件名"]);
    put("draftMailFrom", "メール送信者", fields["メール送信者"]);
    put("draftMailReceivedAt", "メール受信日時", fields["メール受信日時"]);
    put("draftAttachmentFileName", "添付ファイル名", fields["添付ファイル名"]);
    put("draftDownloadedAt", "ダウンロード日", fields["ダウンロード日"]);
...
/* PAYMENT_DOCUMENT_AI_HARD_LINK_DEBUG_20260707_START */
  ...
  function aiHardDebugSetControl(control, label, value) {
    ...
    if (control.tagName === "SELECT") {
      ...
      if (!matched) {
        const debugValue = "__AI_DEBUG__" + label;
        let option = options.find(item => item.value === debugValue);
