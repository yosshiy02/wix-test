...
async function main() {
  const before = await query(`
    SELECT
      ...
      to_regclass('accounting.payable_documents') AS payable_documents_before
  `);
  const backup = await createBackup("before_payables_node_create_" + new Date().toISOString().replace(/[-:.TZ]/g, "").slice(0, 14));
  await query(sql);
