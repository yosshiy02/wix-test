...
if (Test-Path -LiteralPath "G:\GITHUB\wix-test\hd-origin-project\GPTが使う一時ファイルフォルダ\before\web_receiver\public\payables\payment-document-inbox.css") {
  ...
  if (Test-Path -LiteralPath "G:\GITHUB\wix-test\hd-origin-project\web_receiver\public\payables\payment-document-inbox.css") {
    Remove-Item -LiteralPath "G:\GITHUB\wix-test\hd-origin-project\web_receiver\public\payables\payment-document-inbox.css" -Force
    Write-Host "REMOVED: web_receiver\public\payables\payment-document-inbox.css"
  }
}

Write-Host "OK: 支払書類INBOX HTML/CSS分離をUNDOしました。"