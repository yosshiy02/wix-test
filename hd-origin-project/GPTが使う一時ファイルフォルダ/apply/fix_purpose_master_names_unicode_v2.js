...
(async () => {
  ...
  try {
    ...
    const targetRows = await client.query(`
      ...
      WHERE sort_order BETWEEN 10 AND 350
      ORDER BY sort_order, purpose_id
    `);

    lines.push("");
    lines.push("[??????]");
    lines.push(String(targetRows.rows.length));

    const canonicalBySortOrder = new Map();

    for (const row of targetRows.rows) {
      if (!canonicalBySortOrder.has(row.sort_order)) {
        canonicalBySortOrder.set(row.sort_order, row.purpose_id);
      }
    ...
    for (const item of purposes) {
      const existingId = canonicalBySortOrder.get(item.sort_order);
      ...
      if (existingId) {
        const updated = await client.query(
          ...
          `,
          [item.name, item.sort_order, existingId]
        );
    ...
    // ????????????????????????????????
    const inactiveTemp = await client.query(`
      SELECT purpose_id, purpose_name, sort_order, is_active
      FROM expenses.purposes
    ...
    lines.push("");
    lines.push("[??????????????]");
    if (inactiveTemp.rows.length === 0) {
      lines.push("??");
    } else {
      for (const row of inactiveTemp.rows) {
