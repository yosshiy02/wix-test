-- ============================================================
-- HD Origin Project
-- 契約・保険・リース専門解析DB 基礎構築
--
-- 既存 20260708_005 を元に以下を追加
-- 1. 共通専門解析結果 specialist_analysis_id との接続
-- 2. OCR単位の現行下書き1件保証
-- 3. 仕分け下書き単位の現行下書き1件保証
-- ============================================================
BEGIN;
...
CREATE INDEX IF NOT EXISTS idx_cil_drafts_specialist_analysis_id
  ON accounting.payment_document_contract_insurance_lease_drafts(specialist_analysis_id);

CREATE UNIQUE INDEX IF NOT EXISTS uq_cil_drafts_current_ocr
  ON accounting.payment_document_contract_insurance_lease_drafts(payment_document_ocr_import_id)
  WHERE is_current = TRUE
    AND deleted_at IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS uq_cil_drafts_current_sorting
