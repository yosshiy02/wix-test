...
async function hdOriginAskRestoreLatestBackupAfterStartup() {
  ...
  if (!latest) {
    console.log("通常バックアップが見つかりません。このまま使用できます。");
    console.log("");
    return false;
  }
  ...
  console.log("");
  console.log("最新のバックアップをリストアしますか？");
  console.log("リストアすると、現在のDBはバックアップ内容で上書きされます。");
  console.log("実行前に、リストア前保険バックアップを自動作成します。");
  console.log("");

  const first = await hdOriginAskStartupRestore("リストアする場合だけ「はい」と入力してください。しない場合はEnter: ");

  if (first !== "はい") {
    console.log("リストアしません。このまま使用できます。");
    console.log("");
    return false;
  }

  console.log("");
  console.log("本当にリストアしますか？");
  console.log(`対象バックアップ: ${latest.file_name}`);
  console.log("");

  const second = await hdOriginAskStartupRestore("本当にリストアする場合だけ「本当にする」と入力してください。中止はEnter: ");

  if (second !== "本当にする") {
    console.log("リストアを中止しました。このまま使用できます。");
    console.log("");
    return false;
  }

  console.log("");
  console.log("最新バックアップからリストアを開始します。");
  console.log(`対象: ${latest.file_name}`);

  try {
    const result = await restoreBackup(latest.file_name);

    console.log("リストアが完了しました。");
    console.log(`復元ファイル: ${result.restored_file}`);

    if (result.safety_backup && result.safety_backup.file_name) {
      console.log(`リストア前保険バックアップ: ${result.safety_backup.file_name}`);
    }

    await ensureDatabaseReady();

    console.log("DB確認が完了しました。このまま使用できます。");
    console.log("");
    return true;
  } catch (err) {
    console.error("リストアに失敗しました:", err.message);

    if (err.stderr) {
      console.error(err.stderr);
    }

    console.error("リストア失敗後のDB状態を確認してください。");
    console.log("");
    return false;
  }
}
