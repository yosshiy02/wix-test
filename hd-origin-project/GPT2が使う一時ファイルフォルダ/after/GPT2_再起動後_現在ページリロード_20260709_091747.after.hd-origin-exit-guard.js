...
`;
  ...
  function lockOperation() {
    locked = true;
    restartPageUrl = window.location.href;
  ...
  function showBackupComplete(message) {
    ...
    setOverlay(
      ...
      [
        ...
        "",
        "サーバーを再起動しています。",
        "再起動後、この画面を自動で再読み込みします。"
      ].join("\n"),
      "busy"
    );

    setTimeout(() => {
      waitForServerThenReloadCurrentPage(0);
    }, 3500);
  }
  ...
  function waitForServerThenReloadCurrentPage(attempt) {
    const target = restartPageUrl || window.location.href;
    const checkUrl = new URL(target, window.location.href);
    checkUrl.searchParams.set("_hd_origin_restart_probe", String(Date.now()));
    ...
    fetch(checkUrl.toString(), {
      ...
      .catch(() => {
        if (attempt >= 90) {
          showBackupError("サーバー復帰確認ができませんでした。画面を手動で更新してください。");
          return;
        }

        setTimeout(() => {
          waitForServerThenReloadCurrentPage(attempt + 1);
        }, 1000);
      });
  }
