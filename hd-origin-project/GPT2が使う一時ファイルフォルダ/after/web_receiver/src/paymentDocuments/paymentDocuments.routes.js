...
function hdOriginResolvePaymentDocumentAnalysisSystem(source = {}) {
  ...
  const explicitCode = hdOriginPaymentDocumentAnalysisSystemText(
    ...
    s.analysisSystemCode ||
    summary.analysis_system_code ||
    ""
  );
  ...
  const explicitLabel = hdOriginPaymentDocumentAnalysisSystemText(
    ...
    s.analysisSystemLabel ||
    summary.analysis_system ||
    summary.analysis_system_label ||
    ""
  );

  const explicitReason = hdOriginPaymentDocumentAnalysisSystemText(
    s.analysis_system_reason ||
    s.analysisSystemReason ||
    summary.analysis_system_reason ||
    s.review_reason ||
    s.reason ||
    summary.reason ||
    ""
  ...
  const explicitConfidence = hdOriginPaymentDocumentAnalysisSystemConfidence(
    s.analysis_system_confidence ||
    s.analysisSystemConfidence ||
    summary.analysis_system_confidence ||
    s.confidence_label ||
    s.ai_confidence ||
    s.confidence ||
  ...
  if (knownLabels[explicitCode]) {
    return {
      code: explicitCode,
      label: explicitLabel || knownLabels[explicitCode],
  ...
  if (req.method === "POST") {
    ...
    if (sortUrlPath.startsWith("/api/payment-documents/ai-sort/")) {
      try {
        ...
        sortResult = hdOriginPolishPaymentDocumentLeaseSortResult(sortResult, ocrText);
