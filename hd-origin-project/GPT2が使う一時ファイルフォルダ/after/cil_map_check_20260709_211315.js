...
(function () {
  ...
  async function postContractInsuranceLeaseDraftFromSingleSave(sortingPayload, sortingSaved) {
    const payload = {
      ...
      paymentDocumentSortingDraftId: sortingSaved && sortingSaved.paymentDocumentSortingDraftId ? sortingSaved.paymentDocumentSortingDraftId : "",
    ...
    if (!res.ok || !json.ok) {
      throw new Error(json.error || "契約・保険・リース専門下書き保存に失敗しました。");
    }
