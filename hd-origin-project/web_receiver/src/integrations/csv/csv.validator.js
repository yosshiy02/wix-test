...
function validateParsedCsv(profile, parsed, options = {}) {
  ...
  if (includeHeader) {
    if (!Array.isArray(parsed.headers) || parsed.headers.length === 0) {
      ...
      const requiredHeaders = profile.columns.filter(c => c.required).map(c => c.csvHeader);
  ...
  const rowObjects = parsed.rows.map((r) => {
    const byHeader = r.valuesByHeader || {};
    const mapped = {};
    profile.columns.forEach((column) => {
      mapped[column.sourceField] = byHeader[column.csvHeader] ?? '';
    });
    return mapped;
  });
