...
if (req.method === "GET" && pathname === "/api/receipts/imports") {
    /* RECEIPT_HIDE_SAVED_IMPORTS_ROUTE_20260705_START */
    const includeSaved =
      getSearchParam(req, "include_saved") === "1" ||
      getSearchParam(req, "includeSaved") === "1";
    ...
    /* RECEIPT_HIDE_SAVED_IMPORTS_ROUTE_20260705_END */

    /* RECEIPT_HIDE_SAVED_IMPORTS_ROUTE_DOUBLE_FILTER_20260705_START */
    const savedStatuses = new Set([
      "本保存済み",
      "saved",
      "posted",
      "completed"
    ...
    const visibleItems = includeSaved
      ? items
      : (items || []).filter((item) => {
          const status = String(item && item.status ? item.status : "").trim();
          const lower = status.toLowerCase();
          return !savedStatuses.has(status) && !savedStatuses.has(lower);
        });
    /* RECEIPT_HIDE_SAVED_IMPORTS_ROUTE_DOUBLE_FILTER_20260705_END */
  ...
  if (req.method === "GET" && detailMatch) {
    ...
    const drafts = newDraft && Array.isArray(newDraft.details)
      ? newDraft.details.map((detail) => ({
          ...
          line_items: detail.line_items || detail.breakdowns || [],
          lineItems: detail.lineItems || detail.breakdowns || [],
  ...
  if (req.method === "GET" && taxBreakdownMatch) {
    try {
      ...
      const items = await repository.getReceiptDraftDetailTaxBreakdowns(draftId);
  ...
  if ((req.method === "PUT" || req.method === "PATCH") && taxBreakdownMatch) {
    try {
      ...
      const saved = await repository.replaceReceiptDraftDetailTaxBreakdowns(draftId, items);
  ...
  if ((req.method === "PUT" || req.method === "PATCH") && draftMatch) {
    try {
      const body = await readJsonBody(req);
      const saved = await repo.updateReceiptDraftDetail(Number(draftMatch[1]), body);
  ...
  /* RECEIPT_POST_SAVE_ROUTES_20260705_START */
  const bulkPostMatch = pathname === "/api/receipts/imports/bulk-post";

  if (req.method === "POST" && bulkPostMatch) {
    try {
      const body = await readJsonBody(req);
      const rawIds = Array.isArray(body.receiptImportIds)
        ? body.receiptImportIds
        : Array.isArray(body.receipt_import_ids)
          ? body.receipt_import_ids
      ...
      for (const id of ids) {
        try {
          const result = await repo.postReceiptDraftByImportId(id, {
            mode: "bulk"
          });

          results.push(result);

          if (result && result.ok && result.already_saved) {
            alreadySaved++;
      ...
      sendJson(res, 500, {
        ok: false,
        error: error.message || "まとめて本保存に失敗しました。"
      });
  ...
  const singlePostMatch = pathname.match(/^\/api\/receipts\/imports\/(\d+)\/post$/);

  if (req.method === "POST" && singlePostMatch) {
    try {
      const result = await repo.postReceiptDraftByImportId(Number(singlePostMatch[1]), {
        mode: "single"
      });
      ...
      sendJson(res, 500, {
        ok: false,
        error: error.message || "本保存に失敗しました。"
      });
