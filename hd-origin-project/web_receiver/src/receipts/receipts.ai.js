...
analyzeReceiptImport = async function analyzeReceiptImportWithMasterHints(receiptImport) {
  ...
  const instruction = [
    ...
    "摘要分類は、OCRから確認できる取引内容を簿記実務の通常の処理に当てはめ、候補の中から一般的に最もよく使われる標準分類を優先してください。",
    "特殊・限定的な摘要分類は、その分類を選ぶ明確な根拠がOCR本文にある場合だけ選んでください。",
    "取引内容から標準的な摘要分類を選べる場合は、目的や利用事情が不明でも要確認に逃げず、その標準分類を選んでください。",
